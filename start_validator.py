...
async def auto_update(config):
    while True:
        latest_version = utils.get_version()
        if cortext.__version__ != latest_version:
            bt.logging.info(f"Updating to version {latest_version}")
...
def main():
    ...
    loop.create_task(auto_update(config))
